...
#arguments will include (this will be very similar to Fitelson; the difference is they use the difference measure of confirmation)
# - BN
# - narrationNodes
# - states of narrationNodes


DouvenMeijsCoherenceForBNs <- function (BN, narrationNodes, states){
...
#  combinations
  ...
  combinations$DiffLR <- combinations$PrLR - combinations$PriorR
  combinations$DiffRL <- combinations$PrRL - combinations$PriorL
  ...
  Result <- list("Narration Prior" = narrationPrior, "Pairs and measures" = combinations, "DM coherence"  = mean(c(combinations$DiffLR,combinations$DiffRL)), "Computation time" = elapsedTime)
return(Result)
}
