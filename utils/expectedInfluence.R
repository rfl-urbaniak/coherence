...
pr[s] <- as.numeric(querygrain(JN, nodes = consequent)[[1]][as.character(variants$P[s])])
}
variants <- cbind(variants, priorCons = pr)

for (s in 1:length(consequentStates)){
 variants <- cbind(variants, pr[s])  
}
...
posteriors[row]   <-   querygrain(JNtemp, nodes = consequent)[[1]][as.character(variants$P[row])]
}
