...
One issue with this account is that Thaggard insists that the binary notion of coherence is fundamental, while the notion of a coherence of a set of propositions is derivative with respect to this notion. Our considerations so far, however, especially those related to The Beatles scenario, suggest that this pairwise coherence might not be a sufficiently rich notion to allow us to explicate the notion of a coherence of a set of propositions.\footnote{In fact, we argued, even thinking about the object whose coherence is to be evaluated as a set of propositions is too simplistic, as this approach ignores the causal and explanatory structure involved in a narration.}

Another problem is that it is hard to put the framework to actual use. For instance, the theory tells us that global coherence is a function of pairwise coherence, but tells us neither what object a pairwise coherence is, nor what function exactly is to be employed and why.

Further, the idea that coherence is inversely proportional to the number of propositions involved in an explanation is---presumably---some sort of a simplicity criterion. Alas, it is very crude: there are multiple ways a description of events can be cut into propositions, and merely counting propositions does not provide an enlightening measure of the complexity of an explanation.}





\item{\citet{vlek2013modeling} used Bayesian networks to develop a method of  modeling stories and narrations in legal contexts. On this approach,  the coherence of a story (represented by a whole Bayesian network)
is captured by the addition of a single narration root node. This root node  becomes an ancestor node to all the other nodes such that the conditional probability of each dependent node given that the state of this root is 1 (that is, the
corresponding proposition is assumed to be true), is also 1.\footnote{See \citep{vlek2014building,vlek2015,vlek2016method,vlek2016stories} for more details, and \citep{fenton2013general} for another take on Bayesian network representation of narrations.}  Then, the prior probability  probability  of the scenario node is identified with coherence. The idea here is that parts of a scenario are supposed to support each other, and by adding the scenario node this is ensured as now, via the scenario node, an increase in the probability of one of the elements will transfer to an increase of the probability of the other elements. CITE ARGUMENTS SCENARIOS AND PROBABILITIES 
...
\bibliography{coherence,papers11ERC}
